...
async def register_vobiz_credentials() -> None:
    api_key = os.getenv("ULTRAVOX_API_KEY", "").strip()
    auth_id = os.getenv("VOBIZ_AUTH_ID", "").strip()
    auth_token = os.getenv("VOBIZ_AUTH_TOKEN", "").strip()
    ...
    print("Registering Vobiz credentials with Ultravox...")
    ...
    if response.status_code in (200, 201):
        print("✅ Vobiz credentials registered successfully!")
        print("   You can now make outbound calls using Vobiz SIP Trunk.")
...
if __name__ == "__main__":
    asyncio.run(register_vobiz_credentials())